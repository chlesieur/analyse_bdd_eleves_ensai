...
nb_eleve_par_annee <- bdd %>% 
  distinct(id_etudiant, annee, annee_ecole, redoublement) %>%
...
# retirer préfixe "nb_" pour l'affichage dans la légende via labels
ggplot(
  taux_pct %>%
    mutate(spe_entree = factor(spe_entree, levels = ordre)),
  aes(x = bloc_an, y = pct, fill = spe_entree)  # On garde fill = spe_entree
) +
  ...
  scale_fill_manual(
    values = blues_palette,  # Palette bleue personnalisée
    name = "Voie d'entrée",
    labels = function(x) sub("^nb_", "", x)  # Nettoyer les noms
  ) +
  labs(
  ...
  theme(
    ...
    legend.position = "right",
    legend.key.size = unit(0.5, "cm")  # Ajuste la taille des carrés de légende
  )
```
...
nb_eleve_1A <- bdd %>% 
  ...
  mutate(
    ...
    att_ing = factor(att_ing, levels = c("attaché", "ingénieur"))
  ) %>%
...
ggplot(
  ...
  labs(
    title = "Part des élèves en difficultés (< 12/20) par voie d'entrée (1A)",
    subtitle = "Dégradé de bleu : du plus ancien (clair) au plus récent (foncé)",
    x = "Voie d'entrée",
    y = NULL
  ) +
  theme_minimal() +
  theme(
    ...
    axis.text.x = element_blank(),   # enlève les valeurs de l'axe horizontal
    axis.ticks.x = element_blank()   # enlève les graduations
  )
...
ggplot(bdd_spécial_mat_periode, aes(x = spe_entree, y = MSS1, fill = bloc_an)) +
  ...
  labs(
    title = "Réussite en matières statistiques au S1 en 1A selon la filière d'origine en évolution",
    x = "Filière d'origine",
    y = "Note en mathématiques",
    fill = "Bloc"
  ) +
