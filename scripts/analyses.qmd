...
# Récupère le chemin de la bibliothèque renv
renv_lib <- renv::paths$library()

# Vérifie les packages dans renv uniquement
missing <- pkgs[!vapply(pkgs, function(pkg) {
  file.exists(file.path(renv_lib, pkg))
}, logical(1))]

if (length(missing)) {
  stop("Packages manquants dans renv: ", paste(missing, collapse = ", "), call. = FALSE)
}

# Charge les packages depuis renv
invisible(lapply(pkgs, function(pkg) {
  library(pkg, lib.loc = renv_lib, character.only = TRUE)
}))
...
```{r}
bdd <- read_parquet(here("data","bdd_2015_2025.parquet")) %>% filter(annee_ecole != "Autres")
