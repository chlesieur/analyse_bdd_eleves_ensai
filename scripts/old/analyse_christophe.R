...
########## Choix du statut (intgénieur ou fonctionnaire) selon le classement au concours maths ########
# travaux sur 1A 
...
# Évolution du rang moyen par année selon le choix de statut
...
ggplot(bdd_plot, aes(x = annee, y = rang_med_math, 
                     color = statut_etudiant, group = statut_etudiant)) +
  geom_line(size = 1) +
  geom_point(size = 2) +
  labs(
    title = "Évolution du rang médian au concours d'entrée spé maths par statut étudiant",
    x = "Année",
    y = "Rang moyen (%)",
    color = "Statut étudiant"
  ) +
